...
const calculateWeeklyHours = (pattern: string, shiftDuration: number): { avgHours: number; difference: number } => {
    ...
    const weeklyHoursContract = 37.5; // Default contract hours
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70 p-4 backdrop-blur-sm">
            <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col text-gray-100 overflow-hidden">
                <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-800">
                    <div>
                        <h2 className="text-xl font-bold text-white">Gerador de Horarios</h2>
                        <p className="text-sm text-gray-400">Opcoes geradas para 5 equipas com cobertura total 24h</p>
                    </div>
                    <button 
                        onClick={onClose} 
                    ...
                    <div className="w-full lg:w-80 bg-gray-900 p-6 border-b lg:border-b-0 lg:border-r border-gray-800 overflow-y-auto shrink-0">
                        <div className="mb-6 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                            ...
                            <ul className="text-xs text-gray-300 space-y-1.5">
                                ...
                                <li className="flex justify-between"><span>Duracao Turno:</span> <span className="text-white font-mono">8h45 (8.75h)</span></li>
                                <li className="flex justify-between"><span>Alvo Semanal:</span> <span className="text-white font-mono">37.5h</span></li>
                            </ul>
                        </div>

                        <h3 className="font-semibold text-gray-300 mb-4 uppercase text-xs tracking-wider">Preferencias Ajustaveis</h3>
                        <div className="space-y-6">
                            <div>
                                <label htmlFor="max-work" className="block text-sm font-medium text-gray-400 mb-2">
                                ...
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        aria-label="Maximo dias de trabalho seguidos"
                                    />
                                    <span className="text-xl font-bold text-white w-8 text-center" aria-hidden="true">{constraints.maxConsecutiveWork}</span>
                                </div>
                            </div>

                            <div>
                                <label htmlFor="max-off" className="block text-sm font-medium text-gray-400 mb-2">
                                    Max. Folgas Seguidas
                                </label>
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        className="flex-1 accent-blue-500 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer"
                                        aria-label="Maximo folgas seguidas"
                                    />
                                    <span className="text-xl font-bold text-white w-8 text-center" aria-hidden="true">{constraints.maxConsecutiveOff}</span>
                                </div>
                            </div>

                            <div>
                                <label htmlFor="min-block" className="block text-sm font-medium text-gray-400 mb-2">
                                    Tamanho Min. Blocos
                                </label>
                                ...
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        className="flex-1 accent-blue-500 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer"
                                        aria-label="Tamanho minimo dos blocos"
                                    />
                                    <span className="text-xl font-bold text-white w-8 text-center" aria-hidden="true">{constraints.minBlockSize}</span>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                aria-label={isGenerating ? 'A gerar horarios' : 'Gerar horarios'}
                            >
                                {isGenerating ? 'A Gerar...' : 'Gerar Opcoes'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 bg-gray-800/50 p-6 overflow-y-auto">
                        ...
                        {isGenerating && results.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-64 text-blue-400">
                                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mb-4" aria-hidden="true"></div>
                                <p>A procura de padroes matematicos...</p>
                            </div>
                        )}

                        <div className="grid grid-cols-1 gap-4">
                            {results.map((res, idx) => {
                                const { avgHours, difference } = calculateWeeklyHours(res.pattern, 8.75);
                                return (
                                    <div key={idx} className="bg-gray-800 border border-gray-700 rounded-lg p-4 hover:border-blue-500 transition-colors group">
                                        <div className="flex items-start justify-between mb-3">
                                            ...
                                            <button
                                                ...
                                                aria-label={`Usar horario ${generateCreativeName(idx, res.cycleLength)}`}
                                            >
                                                Usar Este
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex gap-4 text-xs text-gray-500 border-t border-gray-700 pt-3 mt-2">
                                            <div>
                                                <span className="block text-gray-400">Media Trabalho</span>
                                                <span className="font-mono text-white">{res.quality.avgWorkBlock.toFixed(1)} dias</span>
                                            </div>
                                            <div>
                                                <span className="block text-gray-400">Media Folga</span>
                                            ...
                                            <div>
                                                <span className="block text-gray-400">Media Semanal</span>
                                                <span className="font-mono text-blue-300">
                                                    {avgHours.toFixed(1)}h{' '}
                                                    <span className={`text-[10px] ${Math.abs(difference) <= 0.5 ? 'text-green-400' : 'text-gray-500'}`}>
